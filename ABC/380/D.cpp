...
char solve(string& S, ll K) {
	ll N = S.size();
	//文字の種類を大文字か小文字かが別々に当てれればいい

	//文字の種類はSの繰り返し、問題は、それが大文字か小文字か

	//x回目のS[i]なら、(xの立ってるビットの数）%2==0なら小文字、そうじゃないなら大文字？
	...
	if (cnt % 2 == 0) {
		if (islower(ans))return tolower(ans);
		else return toupper(ans);
	}
	else {
		if (islower(ans))return toupper(ans);
		else return tolower(ans);
	}
}
