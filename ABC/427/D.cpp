...
int main() {
    ...
    for (; T--;) {
        ...
        vector<vector<bool>> dp(2 * K + 1, vector<bool>(N + 1, false));
        //dp[k][n]:=残りターン数がkの時、駒がマスnにあった場合、どちらが勝つか(trueの場合Aliceが勝つとする)

        for (int n = 1; n <= N; n++)dp[0][n] = S[n - 1] == 'A';

        for (int k = 1; k <= 2 * K; k++) {
            if (k % 2) {//Boｂのターン
                //一つでも勝ちのパターンがあればそれに進めばいい
