...
void dfs(vector<vector<int>>& G, vector<bool>& visited, vector<int>& col, int& cnt, int pos) {
    visited[pos] = true;

    for (auto nex : G[pos]) {
        if (col[pos] == col[nex])cnt++;
        if (visited[nex])continue;

        visited[nex] = true;
        dfs(G, visited, col, cnt, nex);
    }
...
int main() {
    ...
    for (int s = 0; s < (1 << N); s++) {
        vector<int> col(N + 1, 0);//白...0
        for (int j = 0; j < N; j++)if ((s >> j) & 1)col[j] = 1;//黒...1

        vector<bool> visited(N + 1, false);

        int cnt = 0;
        for (int i = 1; i <= N; i++) {//グラフが連結でないことがあるので全ての頂点からスタートする
            if (visited[i])continue;
            dfs(G, visited, col, cnt, i);
        }
