...
bool check(vector<vector<ll>>& A, ll x,vector<ll>& P) {
    //dp[h][w]...マス（h,w)にたどり着くまでに必要な最小のコインの枚数
    ...
    for (int i = 0; i < H; i++)for (int j = 0; j < W; j++) {
        ...
        if (dp[i][j] < 0) dp[i][j]=-LLONG_MAX/2;//どっちから来ても無理
    }
...
int main() {
    ...
    for (int i = 0; i < H; i++)for (int j = 0; j < W; j++) {//はじめからごはん分のお金を減らしておく
        A[i][j] -= P[i + j];
    }
