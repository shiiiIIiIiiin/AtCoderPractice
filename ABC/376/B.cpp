...
ll solve(int& posL, int& posR, char H, int T, int N) {
	if (H == 'L') {
		int from = min(posL, T), to = max(posL, T);
		posL = T;
		
		if (from < posR && posR < to)return N + from - to;
		else return to - from;
	}
	else {
		int from = min(posR, T), to = max(posR, T);
		posR = T;

		if (from < posL && posL < to)return N + from - to;
		else return to - from;
	}
}
