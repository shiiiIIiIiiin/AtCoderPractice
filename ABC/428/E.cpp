...
void dfs(vector<vector<int>>& G, vector<pair<int, int>>& dp, vector<pair<int, int>>& dp2, int pos, int pre) {

    for (auto nex : G[pos]) {
        if (nex == pre)continue;
        dfs(G, dp, dp2, nex, pos);

        pair<int, int> memo = { dp[nex].first + 1,dp[nex].second };

        if (memo >= dp[pos]) {
            dp2[pos] = dp[pos];
            dp[pos] = memo;
        }
        else if (memo >= dp2[pos]) {
            dp2[pos] = memo;
        }
    }
}

void dfs2(vector<vector<int>>& G, vector<pair<int, int>>& dp, vector<pair<int, int>>& dp2, int pos, int pre, vector<pair<int, int>>& ans,pair<int,int> par) {
    ...
    for (auto nex : G[pos]) {
        ...
        pair<int, int> nex_par;

        if (dp[pos].second == dp[nex].second) {//最長のものが同じ方向の場合、2番目にいいものとparのうちいい方を次の親方向に選ぶ
            nex_par = max(dp2[pos], par);
        }
        else nex_par = max(dp[pos], par);
        
        nex_par.first++;
        dfs2(G, dp, dp2, nex, pos, ans, nex_par);
    }
}
...
int main() {
    ...
    vector<pair<int, int>> dp(N + 1), dp2(N + 1), ans(N + 1, { -1,-1 });
    for (int i = 1; i <= N; i++) {
        dp[i] = { 0,i };
        dp2[i] = { -1,-1 };
    }
