...
void dfs(vector<vector<pair<int, ll>>>& G, vector<pair<ll, int>>& dp, vector<pair<ll, int>>& dp2, vector<ll>& D, int pos, int pre) {

    for (auto [nex, cost] : G[pos]) {
        if (nex == pre)continue;
        dfs(G, dp, dp2, D, nex, pos);

        if (dp[pos].first <= dp[nex].first + cost) {
            dp2[pos] = dp[pos];
            dp[pos] = { dp[nex].first + cost,dp[nex].second };
        }
        else if (dp2[pos].first <= dp[nex].first + cost) {
            dp2[pos] = { dp[nex].first + cost,dp[nex].second };
        }
    }

}

void dfs2(vector<vector<pair<int, ll>>>& G, vector<pair<ll, int>>& dp, vector<pair<ll, int>>& dp2, vector<ll>& D, int pos, int pre, vector<pair<ll, int>>& ans, pair<ll, int> par) {
    
    if (dp[pos].second == pos) {
        ans[pos] = max(dp2[pos], par);
    }
    else {
        ans[pos] = max(dp[pos], par);
    }
    ...
    for (auto [nex, cost] : G[pos]) {
        ...
        if (dp[pos].second == dp[nex].second) {
            nex_par = max(dp2[pos], par);
        }
        else {
            nex_par = max(dp[pos], par);
        }

        nex_par.first += cost;

        dfs2(G, dp, dp2, D, nex, pos, ans, nex_par);
    }
}
...
int main() {
    ...
    vector<pair<ll, int>> dp(N + 1), dp2(N + 1, { -INF,-1 }), ans(N + 1);
