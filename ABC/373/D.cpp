...
void dfs(vector<vector<pair<ll, ll>>>& G, vector<ll>& ans, vector<bool>& visited, int pos) {
	visited[pos] = true;

	for (int i = 0; i < G[pos].size(); i++) {
		ll nex = G[pos][i].first, w = G[pos][i].second;
		if (visited[nex])continue;

		ans[nex] = ans[pos] + w;

		dfs(G, ans, visited, nex);
	}

	//visited[pos] = false;//フラグを元に戻す
}
