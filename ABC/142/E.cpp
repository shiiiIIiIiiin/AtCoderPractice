...
int main() {
    ...
    vector c(M + 1, vector<ll>());

    for (int i = 1; i <= M; i++) {
        cin >> a[i] >> b[i];
        c[i].resize(b[i]);
        for (int j = 0; j < b[i]; j++)cin >> c[i][j];
    }

    ll ans = INF;

    //bitDPを行う？
    //dp[S]...開ける宝箱の集合がSの時のかかるコストの最小値

    vector dp(M + 1, vector<ll>(1 << N, INF));

    //dp[i][mask]:=鍵iまでを使って開けた宝箱の集合がmaskの時に最小コスト


    dp[0][0] = 0;

    vector<ll> cnt(M + 1, 0);
    for (int i = 1; i <= M; i++)for (int j = 0; j < b[i]; j++)cnt[i] |= (1 << (c[i][j] - 1));
    ...
    for (int i = 1; i <= M; i++) {
        for (int j = 0; j < (1 << N); j++) {
            ...
            dp[i][mask] = min(dp[i][mask], dp[i - 1][j] + a[i]);
            dp[i][mask] = min(dp[i][mask], dp[i - 1][mask]);
            
        }
    }

    cout << (dp[M].back() >= INF / 2 ? -1 : dp[M].back()) << endl;
}