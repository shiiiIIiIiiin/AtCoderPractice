...
int main() {
    ...
    LazySegTree sg(500010), sg_sum(500010);

    for (int i = 0; i < N; i++) {
        sg.update(A[i], A[i] + 1, 1);
        sg_sum.update(A[i], A[i] + 1, A[i]);
    }
    ...
    for (; Q--;) {
        ...
        if (t == 1) {
            ...
            sg.update(memo, memo + 1, -1);
            sg_sum.update(memo, memo + 1, -memo);

            sg.update(y, y + 1, 1);
            sg_sum.update(y, y + 1, y);
            A[x - 1] = y;
            ...
            ll sum = sg_sum.query(l, r + 1), cnt_gt = sg.query(r + 1, 500009);

            ll ans = cnt * l + sum + cnt_gt * r;
            cout << ans << endl;
