...
vector<int> topological_sort(vector<int>& cnt_in,vector<vector<int>>& G) {
    vector<int> res;
    queue<int> que;
    int N = cnt_in.size() - 1;

    for (int i = 1; i <= N; i++)if (cnt_in[i] == 0)que.push(i);//初期化
...
void dfs(vector<vector<int>>& G, int pos, vector<bool>& visited,vector<bool>& must_read) {
...
int main() {
    int N; cin >> N;
    //グラフはDAG、順番はトポロジカルソート順、読む必要があるかの判定はDFS

    vector<vector<int>> G(N + 1);
    vector<int> cnt_in(N + 1, 0);//入力次数を数える
    ...
    vector<int> tsort = topological_sort(cnt_in, G);

    
    vector<bool> visited(N + 1);
    vector<bool> must_read(N + 1);
    dfs(G, 1, visited, must_read);


    for (int i = tsort.size() - 1; i >= 0; i--) {
        int k = tsort[i];
        if (k == 1)break;
        if (must_read[k])cout << k << " ";
    }
