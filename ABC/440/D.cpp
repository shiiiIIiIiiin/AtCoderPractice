...
bool check(vector<ll>& A, ll x, ll y, ll s) {
    ll cnt = upper_bound(A.begin(), A.end(), x + s) - lower_bound(A.begin(), A.end(), x);

    return s + 1 - cnt < y;
}

ll solve(vector<ll>& A, ll x, ll y) {
    //決め打ち二分探索をする, Q(s):=x以上x+s以下の数でリストに含まれていない数がy種類以下である

    ll l = -1, r = INF, m;

    while (r - l > 1) {
        m = (r + l) / 2;
        if (check(A, x, y, m))l = m;
        else r = m;
    }
