...
int main() {
    ...
    while (!qt.empty() && !qa.empty()) {
        ...
        ll step = min(st.second, sa.second);

        auto vt = mv[st.first];
        auto va = mv[sa.first];

        ll rv = va.first - vt.first;
        ll cv = va.second - vt.second;

        ll sr = rt - ra;
        ll sc = ct - ca;

        if (rv == 0 && cv == 0) {
            if (sr == 0 && sc == 0) ans += step;
        }
        else if (rv == 0) {
            if (sr == 0 && sc % cv == 0) {
                ll k = sc / cv;
        ...
        qt.front().second -= step;
        qa.front().second -= step;

        if (qt.front().second == 0) qt.pop();
