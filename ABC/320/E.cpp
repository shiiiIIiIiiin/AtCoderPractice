...
int main() {
    ...
    priority_queue<pair<ll, int>, vector<pair<ll, int>>, greater<pair<ll, int>>> events;//列の順番と列二戻る時間
    priority_queue<int, vector<int>, greater<int>> queue;
    for (int i = 1; i <= N; i++)queue.push(i);

    for (int i = 0; i < M; i++) {
        //次に流す前に列に戻る人がいれば戻す
        while (!events.empty() && events.top().first <= T[i]) {
            auto p = events.top(); events.pop();
            queue.push({ p.second });
        }


        if (queue.empty())continue;//並んでる人がいなければ時間を進める

        int geter = queue.top(); queue.pop();
        ans[geter] += W[i];

        events.push({ T[i] + S[i],geter });//取った人を列から抜けさせる
    }
