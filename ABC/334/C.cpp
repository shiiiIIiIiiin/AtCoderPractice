...
int main() {
	...
	//なくしてないやつはそいつどうしでペアを組ませる
	if (K % 2 == 0) {
		ll ans = 0;
		for (int i = 1; i < K; i += 2)ans += A[i] - A[i - 1];
		cout << ans << endl;
		return 0;
	}
	else {
		...
		for (int i = 0; i + 2 < K; i += 2) {
			mae.push_back(mae.back() + A[i + 1] - A[i]);
			ushiro.push_back(ushiro.back() + A[i + 2] - A[i + 1]);
		}

		for (int i = 0; i < K; i++) {
			ll memo;
			if (i % 2 == 0) {
				memo = mae[i / 2] + (ushiro[K / 2] - ushiro[i / 2]);
			}
			else {
				memo = mae[i / 2] + (ushiro[K / 2] - ushiro[i / 2 + 1]) + A[i + 1] - A[i];
			}
