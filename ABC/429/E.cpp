...
class info {
public:
    ll cost1 = INF, cost2 = INF, from1 = -1, from2 = -1;

};

int main() {
    //安全な頂点から多始点BFSして、各頂点について、2番目までに近い安全な頂点までの距離を記録する
    int N, M; cin >> N >> M;
    ...
    vector<info> near_safe(N + 1);

    for (int i = 0; i < S.size(); i++) {
        if (S[i] == 'S') {
            near_safe[i + 1].cost1 = 0;
            near_safe[i + 1].from1 = i + 1;
            q.push({ 0,i + 1,i + 1 });
        }
    }

    while (!q.empty()) {
        auto [cost, pos, from] = q.front(); q.pop();

        if (cost > near_safe[pos].cost1 && cost > near_safe[pos].cost2)continue;

        for (auto nex : G[pos]) {
            if (near_safe[nex].from1 == from || near_safe[nex].from2 == from)continue;

            if (cost + 1 < near_safe[nex].cost1) {
                if (near_safe[nex].from1 != -1 && near_safe[nex].from1 != from) {
                    near_safe[nex].cost2 = near_safe[nex].cost1;
                    near_safe[nex].from2 = near_safe[nex].from1;
                }

                near_safe[nex].cost1 = cost + 1;
                near_safe[nex].from1 = from;

                q.push({ cost + 1,nex,from });
            }
            else if (cost + 1 < near_safe[nex].cost2 && near_safe[nex].from1 != from) {
                near_safe[nex].cost2 = cost + 1;
                near_safe[nex].from2 = from;
                q.push({ cost + 1,nex,from });
            }
        }
    }
    ...
    for (int i = 1; i <= N; i++) {
        if (S[i - 1] == 'D') {
            cout << near_safe[i].cost1 + near_safe[i].cost2 << endl;
        }
    }
}