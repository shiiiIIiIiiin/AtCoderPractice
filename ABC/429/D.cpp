...
int main() {
    ...
    for (auto p : cnt)memo.push_back(p);
    ...
    for (int i = 0; i < 2 * k; i++)S[i + 1] = S[i] + memo[i % k].second;
    ...
    for (int i = 0; i < k; i++) {
        ...
        if (memo[i].first < memo[(i + 1) % k].first)c = memo[(i + 1) % k].first - memo[i].first;
        else c = M - memo[i].first + memo[(i + 1) % k].first;

        ll sum = C + S[i + 1];

        int idx = abs(lower_bound(S.begin() + (i + 1), S.begin() + (i + k + 1), sum) - S.begin());
