...
ll count(ll x, vector<ll>& primes) {
    //xを素因数分解して２つに分解するとき何通りの方法があるか
    ll c = x;
    map<ll, ll> d;
...
int main() {
    ...
    for (int i = 0; i < is_prime.size(); i++)if (is_prime[i])primes.push_back(i);

    for (ll AB = 1; AB < N; AB++) {
        ll T = N - AB;

        ll cntAB = count(AB, primes), cntT = count(T, primes);
