...
int main() {
    ...
    mt19937 gen(0);
    uniform_int_distribution<ll> distrib(0, N - 1);

    for (int T = 0; T < 100; T++) {//100回ほどやる
        ll u= distrib(gen),v= distrib(gen);
        if (u == v)continue;

        ll x1 = P[u].first, x2 = P[v].first, y1 = P[u].second, y2 = P[v].second;

        //直線の方程式は(y-y1)*(x1-x2)=(y1-y2)*(x-x1)
        int cnt = 0;

        for (int i = 0; i < N; i++) {
            ll x = P[i].first, y = P[i].second;

            if ((y - y1) * (x1 - x2) == (y1 - y2) * (x - x1))cnt++;
        }

        if (cnt > N / 2) {
            //出力
            cout << "Yes" << endl;
            cout << y1 - y2 << " " << -x1 + x2 << " " << -x1 * (y1 - y2) + y1 * (x1 - x2) << endl;
            return 0;
        }
    }
