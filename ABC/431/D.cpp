...
int main() {
    ...
    vector<ll> dp(sum + 1, -INF);//dp[w]:=頭の重さがw以下で回収できる嬉しさの最大値
    dp[0] = 0;

    for (int i = 0; i < N; i++) {
        for (int w = sum; w >= W[i]; w--) {
            dp[w] = max(dp[w] + B[i], dp[w - W[i]] + H[i]);
        }

        for (int w = W[i] - 1; w >= 0; w--)dp[w] += B[i];
    }

    ll ans = 0;

    for (int i = 0; i <= sum / 2; i++)ans = max(ans, dp[i]);
