...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        for (int j = 0; j < M + 1; j++) {
            ...
            for (int k = 0; k < A[i]; k++) {//S[i][k]を使うか
                if (j - int(S[i][k].size()) < 0)continue;
                if (T.substr(j - S[i][k].size(), S[i][k].size()) != S[i][k])continue;

                dp[i][j] = min(dp[i][j], dp[i - 1][j - S[i][k].size()] + 1);
            }
        }
    }


    cout << (dp[N].back() > INF / 2 ? -1 : dp[N].back()) << endl;
}