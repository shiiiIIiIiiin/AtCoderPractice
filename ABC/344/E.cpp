...
int main() {
    ...
    A[0] = 0, A[N + 1] = -1;//末尾と先頭を記録する

    unordered_map<ll, pair<ll, ll>> pn;//前後を確認できるようにする

    int Q; cin >> Q;

    pn.reserve(N + Q + 10);
    pn.max_load_factor(0.7);


    pn[0] = { -INF,A[1] };
    pn[-1] = { A[N - 1],-INF };
    for (int i = 1; i <= N; i++)pn[A[i]] = { A[i - 1],A[i + 1] };
    ...
    for (; Q--;) {
        ...
        if (t == 1) {
            ...
            ll x; cin >> x;
            auto u = pn[x];
            pn[u.first].second = u.second;
            pn[u.second].first = u.first;
            pn.erase(x);
