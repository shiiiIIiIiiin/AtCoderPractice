...
void dfs(ll& ans, vector<vector<ll>>& D, ll N, vector<bool>& used,ll score) {
	ans = max(ans, score);
...
int main() {
	...
	int N = N_input + (N_input % 2 == 1 ? 1 : 0);

	vector<vector<ll>> D(N, vector<ll>(N, 0));
	for (int i = 0; i < N_input - 1; i++)for (int j = i + 1; j < N_input; j++) {
		cin >> D[i][j];
		D[j][i] = D[i][j];
	}
