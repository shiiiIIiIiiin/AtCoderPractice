...
int main() {
	int H, W, N; cin >> H >> W >> N;
	vector<set<int>> tate(2 * 100000 + 1), yoko(2 * 100000 + 1);
	...
	for (; Q--;) {
		...
		if (t == 1) {
			int x; cin >> x;
			cout << yoko[x].size() << endl;

			for (auto k : yoko[x])tate[k].erase(x);
			yoko[x].clear();
		}
		else {
			int y; cin >> y;
			cout << tate[y].size() << endl;
			for (auto k : tate[y])yoko[k].erase(y);
			tate[y].clear();
		}
	}
}