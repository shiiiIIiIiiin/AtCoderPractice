...
int main() {
    int N, Q; cin >> N >> Q;
    vector<ll> A(N), B(N), sumA(N + 1), sum3A(N + 1), sumB(N + 1), sum3B(N + 1);

    for (int i = 0; i < N; i++) {
        cin >> A[i];
        sumA[i + 1] += sumA[i] + A[i] * A[i]; sumA[i + 1] %= MOD;
        sum3A[i + 1] += sum3A[i] + A[i] * A[i] * A[i]; sum3A[i + 1] %= MOD;
    }

    for (int i = 0; i < N; i++) {
        cin >> B[i];
        sumB[i + 1] += sumB[i] + B[i] * B[i]; sumB[i + 1] %= MOD;
        sum3B[i + 1] += sum3B[i] + B[i] * B[i] * B[i]; sum3B[i + 1] %= MOD;
    }

    for (; Q--;) {
        int l, r, L, R; cin >> l >> r >> L >> R;
        if ((sumA[r] - sumA[l - 1] + MOD) % MOD == (sumB[R] - sumB[L - 1] + MOD) % MOD && (sum3A[r] - sum3A[l - 1] + MOD) % MOD == (sum3B[R] - sum3B[L - 1] + MOD) % MOD)cout << "Yes" << endl;
        else cout << "No" << endl;
    }
}