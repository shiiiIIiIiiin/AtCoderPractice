...
int main() {
    ...
    ll sum = 0;
    for (int i = 0; i < M; i++)sum += P[i].second;
    ...
    for (int i = 0; i < M; i++) {
        ...
        */
        ...
        if (cnt < P[i + 1].first - P[i].first) {
            cout << -1 << endl;
            return 0;
        }

        //ans+= (cnt-1) + (cnt-2) + ... + (cnt-(P[i+1].first - P[i].first))

        
        ans += (cnt - 1 + cnt - (P[i + 1].first - P[i].first)) * (P[i + 1].first - P[i].first) / 2;

        cnt -= P[i + 1].first - P[i].first;
    }
