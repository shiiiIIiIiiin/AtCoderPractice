...
int main() {
    ...
    for (; T--;) {
        ...
        vector<vector<ll>> dp(H, vector<ll>(1LL << W, INF));

        for (int s = 0; s < (1 << W); s++) {
            bool can = true;
            for (int j = 0; j < W; j++)if (S[0][j] == '.' && ((s >> j) & 1)) {
                can = false;
                break;
            }
            ...
            for (int j = 0; j < W; j++)if (S[0][j] == '#' && ((s >> j) & 1) == 0)dp[0][s]++;
        }
        ...
        for (int i = 1; i < H; i++) {
            for (int s = 0; s < (1 << W); s++) {
                ...
                for (int ps = 0; ps < (1 << W); ps++) {
                    if (dp[i - 1][ps] > INF / 2)continue;

                    bool can = true;

                    for (int j = 0; j < W - 1; j++) {
                        if ((ps >> j) & 1 && (ps >> (j + 1) & 1 && (s >> j) & 1 && (s >> (j + 1)) & 1))can = false;
                    }
                    ...
                    for (int j = 0; j < W; j++) {
                        if (S[i][j] == '#' && ((s >> j) & 1) == 0)cnt++;
                    }

                    dp[i][s] = min(dp[i][s], dp[i - 1][ps] + cnt);
                }
            }
        }

        ll ans = INF;
        for (int j = 0; j < (1 << W); j++)ans = min(ans, dp[H - 1][j]);
