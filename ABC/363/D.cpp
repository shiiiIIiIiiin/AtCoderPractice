...
int main() {
    ...
    while (true) {
       /*
        ...
        cnt>=2の時...10^(cnt//2+1)-10^(cnt//2)個


       */

        if (N > modpow(10LL, cnt / 2 + 1, INF) - modpow(10LL, cnt / 2, INF)) {
            N -= modpow(10LL, cnt / 2 + 1, INF) - modpow(10LL, cnt / 2, INF);
            cnt++;
        }
        else break;
    }
    ...
    ans = memo = to_string(N + modpow(10LL, cnt / 2, INF) - 1);

    if (cnt % 2 == 0)memo.pop_back();
