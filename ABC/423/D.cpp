...
int main() {
    ...
    for (int i = 0; i < N; i++)waiting.push(i);


    ll cur_cnt = 0, cur_time = 0;
    vector<ll> ans(N);


    while (!waiting.empty() && cur_cnt + C[waiting.front()] <= K) {//はじめの人を入れる
        if (waiting.empty())break;
        int nex = waiting.front(); waiting.pop();

        cur_time = max(cur_time, A[nex]);
        ans[nex] = cur_time;


        event a;
        a.time = cur_time + B[nex];
        a.idx = nex;
        a.c = C[nex];
    ...
    while (!pq.empty()) {
        ...
        while (!waiting.empty() && cur_cnt + C[waiting.front()] <= K) {

            int nex = waiting.front(); waiting.pop();

            pq.push({ max(cur_time,A[nex]) + B[nex],nex,C[nex] });
            ans[nex] = max(cur_time, A[nex]);
