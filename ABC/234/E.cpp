...
int main() {
	...
	//等差数リストを作る->そのリストを二分探索する？
	vector<ll> tousasu;

	for (int i = 0; i <= 99; i++)tousasu.push_back(i);//1桁と2桁の整数はすべて等差数

	for (int i = 2; i <= 17; i++) {//何桁の等差数か
		for (int d = -10; d < 10; d++) {//公差がいくつか

			for (int k = 1; k <= 9; k++) {//一番上の桁がいくつか
				ll res = k;
				ll u = k;
				bool f = true;
				...
				if (f)tousasu.push_back(res);
			}
		}
	}

	sort(tousasu.begin(), tousasu.end());

	ll l = -1, r = tousasu.size() + 1, m;
