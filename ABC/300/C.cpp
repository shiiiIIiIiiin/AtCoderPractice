...
bool check(int x, int y, vector<string>& C, int H, int W,int d) {
	if (x - d < 0 || H <= x + d || y - d < 0 || W <= y + d)return false;

	if (C[x + d][y + d] == '#' && C[x + d][y - d] == '#' && C[x - d][y + d] == '#' && C[x - d][y - d] == '#')return true;
	return false;
}
