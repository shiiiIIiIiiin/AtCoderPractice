...
int main() {
    //ダブリンぐする？→どこにバケツがあるかはダブリンぐで分かる
    //ダブリンぐでその経路で水がどれだけ増えるかをpairでもつ？
    ...
    vector <vector<pair<ll, ll>>> dp(30, vector<pair<ll,ll>>(N));

    for (int i = 0; i < N; i++)dp[0][i] = { A[i],i + 1 };

    for (int i = 1; i < 30; i++) {
        for (int j = 0; j < N; j++) {
            dp[i][j] = dp[i - 1][dp[i - 1][j].first];
