...
ll dfs(vector<vector<pair<int, ll>>>& G, vector<ll>& x, int pos, int pre, ll& score) {
	if (G[pos].size() == 1 && pre != -1) {
		return x[pos];
	}
	...
	for (int i = 0; i < G[pos].size(); i++) {
		int nex = G[pos][i].first, w = G[pos][i].second;

		if (nex == pre)continue;

		ll memo = dfs(G, x, nex, pos, score);
		res += memo;
		score += labs(memo * w);
