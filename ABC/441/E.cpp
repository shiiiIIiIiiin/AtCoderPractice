...
ll f(int i, vector<ll>& cntA) {
    //単調性があるから二分探索で調べる
    ll l = -1, r = N, m;

    while (r - l > 1) {
        m = (r + l) / 2;
        if (cntA[m] - (i == 0 ? 0 : cntA[i - 1]) > 0)l = m;
        else r = m;
    }
...
int main() {
    ...
    ll ans = 0;
    vector<ll> cnt(N + 1, 0);//(Aの数)-(Bの数)を累積和で出せるようにしたもの
    //P[r]>P[l-1]となる(l,r)の数を数える,cntの転倒数を数える

    cnt[0] = 500000;
    for (int i = 0; i < N; i++) {
        cnt[i + 1] = cnt[i];
        if (S[i] == 'A')cnt[i+1]++;
        else if (S[i] == 'B')cnt[i+1]--;
    }

    //https://scrapbox.io/468-cp/%E8%BB%A2%E5%80%92%E6%95%B0
    //から流用
 ...
 */
    ...
    for (int i = 0; i <= N; i++) {
        inversion += fwt.sum(0, cnt[i]);//少ないものを数える
        fwt.add(cnt[i], 1);
    }
