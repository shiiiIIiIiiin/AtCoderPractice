...
int main() {
    ...
    //visitedを管理しないbfsをすればいい
    //順路は最高でも10だからvectorで持てばいい

    queue<tuple<ll, ll,vector<ll>>> q;//{頂点,これまでの合計コスト,これまでの道順}
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < G[pos].size(); i++) {
            int nex = G[pos][i].first, new_cost = cost + G[pos][i].second;
            P.push_back(nex);
            q.push({ nex, new_cost, P });
            P.pop_back();
        }
    }
    ...
    for (int i = 0; i < ans.size(); i++) {
        if (S <= ans[i].second && ans[i].second <= T)a.insert(ans[i].first.back());
    }
