...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        dp[i] = dp[i - 1];
        for (int j = P[i]; j <= M; j++)dp[i][j] = max(dp[i][j], dp[i - 1][j - P[i]] + V[i]);
    }
    ...
    for (int j = 0; j <= M; j++) {
        ...
        else if (dp[N][j] == MAX)starts.push_back(j);
    }
    ...
    for (int i = 0; i < starts.size(); i++) {
        q.push({ N,starts[i] });
        visited[N][starts[i]] = true;
    }
    vector<bool> used(N + 1), skipped(N + 1);
    
    while (!q.empty()) {
        auto [n, k] = q.front(); q.pop();
        if (n == 0)continue;

        if (dp[n][k] == dp[n - 1][k]) {//商品nを使わなかった遷移
            skipped[n] = true;
        ...
        if (k - P[n] >= 0 && dp[n][k] == dp[n - 1][k - P[n]] + V[n]) {
            used[n] = true;

            if (!visited[n - 1][k - P[n]]) {
                q.push({ n - 1,k - P[n] });
                visited[n - 1][k - P[n]] = true;
            }
        }
    }
