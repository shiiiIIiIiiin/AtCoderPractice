...
ll mex(int a, int b, int c) {
    for (int i = 0; i <= 3; i++)if (a != i && b != i && c != i)return i;
}

int main() {
    //mex(A,B,C)=0 or 1 or 2 or 3
    ...
    ll ans = 0;
    vector<vector<ll>> imos1(3, vector<ll>(N, 0)), imos2(3, vector<ll>(N, 0));
    //imos1[a][j]:=0以上の整数iで、S[i]=='M'かつi<jかつA[i]==aを満たすiの個数
    //imos2[b][j]:=整数kで、j<kかつS[k]=='X'かつA[k]==bを満たすkの数

    
    for (int i = 1; i < N; i++) {
        
        for (int a = 0; a < 3; a++) {
            imos1[a][i] = imos1[a][i - 1];
        }
    ...
    for (int j = 1; j + 1 < N; j++) {//jを全探索する
        ...
        */
        ...
        for (int a = 0; a < 3; a++)for (int b = 0; b < 3; b++) {
            ans += imos1[a][j] * imos2[b][j] * mex(a, b, A[j]);
        }
