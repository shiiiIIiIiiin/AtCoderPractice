...
public:
    ...
    Point(int _x, int _y, int _cost, int _cnt) {
        x = _x;
        y = _y;
        cost = _cost;
        cnt = _cnt;
...
int main() {
    ...
    vector<vector<ll>> dpe(H, vector<ll>(W, INF)), dpo(H, vector<ll>(W, INF));
    
    //dpe[i][j]:=偶数回スイッチを踏んで(i,j)に到達する　、　dpo[i][j]:=奇数回スイッチを踏んで(i,j)に到達する　最小距離
    ...
    while (!q.empty()) {
        ...
        ll x = k.x, y = k.y, cost = k.cost, cnt = k.cnt;

        if (cnt % 2 == 0) {//スイッチを踏んだ回数が偶数回
            for (int d = 0; d < 4; d++) {
                int nx = x + dx[d], ny = y + dy[d];
                if (nx < 0 || H <= nx || ny < 0 || W <= ny)continue;
                if (A[nx][ny] == '#')continue;

                if (A[nx][ny] == 'x')continue;//扉が閉じているならスキップ
                ...
                if (A[nx][ny] == '?') {
                    ...
                    /*
                    Point qq(nx, ny, cost + 1, cnt + 1);
                    q.push(qq);
                    dpo[nx][ny] = min(dpo[nx][ny], dpe[x][y] + 1);
                    */

                    if (dpo[nx][ny] > dpe[x][y] + 1) {
                        dpo[nx][ny] = dpe[x][y] + 1;
                        q.push(Point(nx, ny, dpe[x][y] + 1, cnt + 1));
                    }
                }
                else {
                    /*
                    Point qq(nx, ny, cost + 1, cnt);
                    q.push(qq);
                    dpe[nx][ny] = min(dpe[nx][ny], dpe[x][y] + 1);
                    */
            ...
            for (int d = 0; d < 4; d++) {
                ...
                if (A[nx][ny] == 'o')continue;//扉が閉じているのでスキップ
                ...
                if (A[nx][ny] == '?') {
                    ...
                    Point qq(nx, ny, cost + 1, cnt + 1);
                    q.push(qq);
                    dpe[nx][ny] = min(dpe[nx][ny], dpo[x][y] + 1);
                    */

                    if (dpe[nx][ny] > dpo[x][y] + 1) {
                        dpe[nx][ny] = dpo[x][y] + 1;
                        q.push(Point(nx, ny, dpo[x][y] + 1, cnt + 1));
                    }
                    ...
                    Point qq(nx, ny, cost + 1, cnt);
                    q.push(qq);
                    dpo[nx][ny] = min(dpo[nx][ny], dpo[x][y] + 1);
                    */
    ...
    if (dpe[gx][gy] == INF && dpo[gx][gy] == INF)cout << -1 << endl;
    else cout << min(dpe[gx][gy], dpo[gx][gy]) << endl;
