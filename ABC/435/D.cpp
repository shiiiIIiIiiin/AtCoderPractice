...
void dfs(vector<vector<int>>& G, vector<bool>& visited, int pos) {
...
int main() {
    ...
    int Q; cin >> Q;
    vector<bool> black(N + 1, false), visited(N + 1, false);
    
    for (; Q--;) {
        int t, v; cin >> t >> v;

        if (t == 1) {
            //vを黒にする、vから辿って行ける全ての点でかつ黒のものを管理する
            black[v] = true;
