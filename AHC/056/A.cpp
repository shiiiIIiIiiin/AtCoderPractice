...
int main() {
    ...
    StrategyOutput chosen_strategy_A;
    int chosen_i = 0;
    if (K > 1) {
        // 試行: i = K-1 .. 1 の順で、target[K-i] から target[K-1] をでたらめにする
        int original_t_last = t_last_stop_global;
        for (int i = K - 1; i >= 1; --i) {
            int startIndex = (K - 1) - i; // target index where random-walk begins
            if (startIndex < 0 || startIndex >= K) continue;

            // 制限位置を一時的に変更して振る舞い圧縮を行う
            t_last_stop_global = t_at_target[startIndex];
            StrategyOutput cand = generate_strategy(true, N, X, full_path, v, h, K);

            bool cand_success = false;
            if (cand.M > 1 && (size_t)t_last_stop_global < behavior_id_of_t.size() && behavior_id_of_t[0] != behavior_id_of_t[t_last_stop_global]) {
                cand_success = run_simulation(cand, N, T, K, target, v, h);
            }

            if (cand_success) {
                chosen_strategy_A = cand;
                chosen_i = i; // largest i found because we iterate descending
                break;
            }
        }
        // restore
        t_last_stop_global = original_t_last;
    ...
    // --- 最終出力 ---
        if (use_strategy_A) {
            if (chosen_i > 0) {
                // 既に見つかった最長区間の戦略Aをそのまま出力
                print_strategy(chosen_strategy_A);
            } else {
                // フォールバック: 通常の strategy A を生成して出力
