...
// 複数シードで平均スコアを計算
static int evaluateMultiSeed(const Params& p, const vector<int>& seeds) {
    int total = 0;
    for (int seed : seeds) {
        total += simulate(p, false, seed).score;
    }
    return total / (int)seeds.size();
}
...
int main() {
    ...
    const double timeLimitSec = 1.85;
    const double t0 = 15.0;
    const double t1 = 0.5;

    // Phase 1: 全戦略を複数シードで評価してTop3を選ぶ
    vector<pair<int, int>> scores; // (score, strategy_index)
    for (size_t i = 0; i < strategies.size(); i++) {
        int s = evaluateMultiSeed(strategies[i], evalSeeds);
        scores.push_back({s, (int)i});
    }
    sort(scores.rbegin(), scores.rend());

    // Phase 2: Top3戦略でSAを実行
    const int topK = 3;
    ...
    for (int ti = 0; ti < topK && ti < (int)scores.size(); ti++) {
        ...
        Params best = cur;
        int curScore = evaluateMultiSeed(cur, evalSeeds);
        ...
        while (true) {
            ...
            int nxtScore = evaluateMultiSeed(nxt, evalSeeds);
    ...
    // 最終出力: 複数シードで試して最高のものを選ぶ
    int bestFinalScore = -1;
    int bestFinalSeed = evalSeeds[0];
    for (int seed : evalSeeds) {
        int s = simulate(globalBest, false, seed).score;
        if (s > bestFinalScore) {
            bestFinalScore = s;
            bestFinalSeed = seed;
        }
    ...
    SimResult result = simulate(globalBest, true, bestFinalSeed);
