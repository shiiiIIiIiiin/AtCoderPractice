...
struct Params {
    ...
    bool operator<(const Params &other) const {
        if (lenLong != other.lenLong) return lenLong < other.lenLong;
        if (lenMid != other.lenMid) return lenMid < other.lenMid;
        if (invMid != other.invMid) return invMid < other.invMid;
        if (lenShort != other.lenShort) return lenShort < other.lenShort;
        if (invShort != other.invShort) return invShort < other.invShort;
...
int main() {
    ...
    // ===== Phase 1: Beam Search (0.8 sec) =====
    const double beamTimeLimit = 0.8;
    ...
    if (beam.size() > beamWidth) beam.resize(beamWidth);
    ...
    while (true) {
        ...
        for (auto &[curScore, curParams] : beam) {
            for (int trial = 0; trial < 3; trial++) {
                ...
                if (which == 0) nxt.lenLong = clampInt(nxt.lenLong + delta, 6, 9);
                else if (which == 1) nxt.lenMid = clampInt(nxt.lenMid + delta, 4, 7);
                else if (which == 2) nxt.invMid = clampInt(nxt.invMid + delta * 2, 8, 16);
                else if (which == 3) nxt.lenShort = clampInt(nxt.lenShort + delta, 2, 5);
                else if (which == 4) nxt.invShort = clampInt(nxt.invShort + delta * 2, 5, 12);
                else nxt.rProb = clampInt(nxt.rProb + delta * 5, 30, 90);

                if (nxt.lenMid > nxt.lenLong) nxt.lenMid = nxt.lenLong;
                if (nxt.lenShort > nxt.lenMid) nxt.lenShort = nxt.lenMid;
        ...
        for (auto &p : beam) nextBeam.push_back(p);
    ...
    // ===== Phase 2: Simulated Annealing (1.0 sec) =====
    const double saTimeLimit = 1.0;
    ...
    while (true) {
        auto now = chrono::high_resolution_clock::now();
        double elapsed = chrono::duration<double>(now - globalStart).count();
        if (elapsed >= beamTimeLimit + saTimeLimit) break;
