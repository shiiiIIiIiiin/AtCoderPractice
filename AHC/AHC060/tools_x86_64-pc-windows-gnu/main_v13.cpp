...
static SimResult simulate(const Params& p, bool recordActions, int seed = 12345) {
    ...
    while (usedSteps < T - 10) {
        ...
        // ★改善2: 動的lookBias（コーンが長いほど先読み強化）
        int dynamicLookBias = p.baseLookBias;
        if (coneLen >= 4) dynamicLookBias = min(100, p.baseLookBias + 20);
        if (coneLen >= 6) dynamicLookBias = 100;  // 緊急モード
        ...
        if (!cone.empty()) {
            ...
            if (bestShop != -1) {
                next = bestShop;
            } else if ((rng() % 100) < dynamicLookBias) {
                // ★改善3: コーンが長い場合は深い先読み
                int effectiveDepth = p.lookDepth;
                if (coneLen >= 5) effectiveDepth = min(5, p.lookDepth + 1);

                vector<int> goodMoves = findDeliverableMoves(
                    curPos, cone, candidates, shopInventory, isRed, effectiveDepth);
                if (!goodMoves.empty()) {
        ...
        // ★改善4: より賢いR変換（コーンが空でかつ隣接店が多い木で変換）
        if (curPos >= K && !isRed[curPos]) {
            bool shouldConvert = false;
            if (p.rProb > 0 && rng() % p.rProb == 0) {
                shouldConvert = true;
            }
            // コーンが長くなりすぎて配達困難な場合、R変換を促進
            if (cone.size() >= 5 && !isRed[curPos] && (rng() % 30 == 0)) {
                shouldConvert = true;
            }
...
int main() {
    ...
    const double timeLimitSec = 1.00;  // TLE対策でさらに短縮
    const double t0 = 15.0;
    const double t1 = 0.5;

    // Phase 1: 全戦略を1回ずつ評価してTop3を選ぶ
    ...
    for (int ti = 0; ti < topK && ti < (int)scores.size(); ti++) {
        ...
        while (true) {
            ...
            if (which == 0) {
                ...
                nxt.baseLookBias = clampInt(nxt.baseLookBias + delta * 10, 0, 100);
            }
    ...
    cerr << "Params: rProb=" << globalBest.rProb << " minLen=" << globalBest.minLen
         ...
         << " baseLookBias=" << globalBest.baseLookBias << "\n";
