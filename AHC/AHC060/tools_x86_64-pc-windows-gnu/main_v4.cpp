...
// 各店からの最短距離
vector<vector<int>> distFromShop;

void precomputeDistances() {
    distFromShop.assign(K, vector<int>(N, 1e9));

    for (int shop = 0; shop < K; shop++) {
        queue<int> q;
        q.push(shop);
        distFromShop[shop][shop] = 0;
        while (!q.empty()) {
            int u = q.front(); q.pop();
            for (int v : adj[u]) {
                if (distFromShop[shop][v] > distFromShop[shop][u] + 1) {
                    distFromShop[shop][v] = distFromShop[shop][u] + 1;
                    q.push(v);
                }
            }
        }
...
struct Params {
    ...
    int guideBias;  // 店方向への誘導確率(0-100) ← 新パラメータ
};
...
static SimResult simulate(const Params& p, bool recordActions) {
    ...
    while (usedSteps < T - 10) {
        ...
        if (!cone.empty()) {
            ...
            if (bestShop != -1) {
                next = bestShop;
            } else if ((rng() % 100) < p.guideBias) {
                // === 3. 配達先がないなら、配達可能な店の方向へ緩やかに誘導 ===
                int targetShop = -1;
                int minDist = 1e9;
                for (int shop = 0; shop < K; shop++) {
                    if (!shopInventory[shop].count(cone)) {
                        if (distFromShop[shop][curPos] < minDist) {
                            minDist = distFromShop[shop][curPos];
                            targetShop = shop;
                        }
                    }
                }
                ...
                if (targetShop != -1) {
                    ...
                    int curDist = distFromShop[targetShop][curPos];
                    for (int v : candidates) {
                        if (distFromShop[targetShop][v] < curDist) {
                            goodCandidates.push_back(v);
                        }
                    }
                    if (!goodCandidates.empty()) {
                        next = goodCandidates[rng() % goodCandidates.size()];
                    }
                }
            }
        }
...
int main() {
    ...
    // 初期パラメータ（main.cppベース + guideBias追加）
    Params cur{50, 1, 75, 50};  // rProb, minLen, treeBias, guideBias
    Params best = cur;
    ...
    while (true) {
        ...
        if (which == 0) {
            ...
            nxt.guideBias = clampInt(nxt.guideBias + delta * 5, 0, 100);
        }
    ...
    cerr << "Params: rProb=" << best.rProb << " minLen=" << best.minLen
         << " treeBias=" << best.treeBias << " guideBias=" << best.guideBias << "\n";
