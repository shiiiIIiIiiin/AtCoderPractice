...
int main() {
    ...
    for (int s = 1; s < (1 << H); s++) {
        vector<int> gyo;
        for (int j = 0; j < H; j++)if ((s >> j) & 1)gyo.push_back(j);

        map<ll, ll> cnt;
        for (int j = 0; j < W; j++) {
            bool f = true;
            for (int k = 0; k < gyo.size() - 1; k++) {
                if (P[gyo[k]][j] != P[gyo[k + 1]][j])f = false;
            }
        ...
        ll memo = -1;
        for (auto p : cnt)memo = max(memo, p.second);

        ans = max(ans, ll(memo * gyo.size()));
    }
