...
int main() {
    ...
    vector<vector<ll>> imos(5010, vector<ll>(5010, 0));

    for (int i = 0; i < N; i++)imos[P[i].first][P[i].second]++;
    for (int i = 0; i < 5010; i++) {
        for (int j = 1; j < 5010; j++)imos[i][j] += imos[i][j - 1];
    }

    for (int j = 0; j < 5010; j++) {
        for (int i = 1; i < 5010; i++)imos[i][j] += imos[i - 1][j];
    }

    ll ans = -1;

    for (int a = 1; a < 5010; a++)for (int b = 1; b < 5010; b++) {
        int s = min(5009, a + K), t = min(5009, b + K);

        ans = max(ans, imos[s][t] - imos[a - 1][t] - imos[s][b - 1] + imos[a - 1][b - 1]);
    }
