...
int main() {
    int N, S; cin >> N >> S;
    vector<ll> A(N + 1); for (int i = 0; i < N; i++)cin >> A[i + 1];

    int MAXS = 10010;
    vector<vector<bool>> dp(N + 1, vector<bool>(MAXS, false));

    dp[0][0] = true;

    for (int i = 1; i <= N; i++)for (int j = 0; j < MAXS; j++) {
        // dp[i][j] = dp[i - 1][j] || dp[i - 1][j - A[i]]; ←基本はこう、ただし、j - A[i] < 0 の場合を除外する必要がある
        dp[i][j] = dp[i - 1][j];
